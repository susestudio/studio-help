...
The Preboot Execution Environment (PXE) is a boot protocol implemented
in most BIOS implementations. 
The protocol sends DHCP requests to assign an IP address. 
After it got an IP address, it uses TFTP to download kernel and boot
instructions.
...
be copied at specific locations of a network boot server. 
...
Enterprise 10.
For SUSE Linux Enterprise 10 you should use one of alternative
...
2. Edit the file '/etc/sysconfig/atftpd'. Set or modifiy the following
variables:
...
domain settings, all these parameters needs to be adapted in order to
...
3. Edit the file '/etc/sysconfig/dhcpd' and setup the network
...
In order to make use of the image, all related parts needs to be
...
However, as soon as Linux takes over control, the download protocols
http, https, and ftp are supported too.
In order to use these additional protocols, use the following kernel
parameters:
...
Find more details in the KIWI documentation (package kiwi-doc).
